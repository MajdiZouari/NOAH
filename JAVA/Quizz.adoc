...
== Object oriented programming basics
  ...
  *  What is the Hashcode method used for in Java?
  -----------------
  Le Hashcode est une représentation numérique du 'contenu' de l'objet. Il sert à représenter l'objet dans les collections hashées (hashset, hashmap). Deux Objets égaux ont le même hashcode. De ce fait, il sert de clé d'unicité dans ces collections. 
